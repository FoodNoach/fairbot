...
module.exports.run = async(bot, message, args) => {
    ...
    if(!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send("U heeft geen permissies dit commando uit te voeren!");

    if(!message.guild.me.hasPermission("BAN_MEMBERS")) return message.channel.send("Ik heb geen permissies hiervoor!");
    ...
    if(banUser.hasPermission("MANAGE_MESSAGES")) return message.channel.send("Sorry u kunt deze gebruiker geen ban geven!");
    ...
    message.channel.send(embedPromt).then(async msg =>{
        ...
        if(emoji === "✅"){
            ...
            banUser.ban(reason).catch(err => { 
                if (err) return message.reply("Er is een fout opgetreden!");
            });
            ...
            return message.reply("De ban is geanuleerd!").then(m => m.delete(5000));
